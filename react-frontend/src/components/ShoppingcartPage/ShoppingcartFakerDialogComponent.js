...
import shoppingcartFakerFactory from "./shoppingcartFakerFactory";
...
const ShoppingcartFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = shoppingcartFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("shoppingcart").create(elem));
...
export default connect(null, mapDispatch)(ShoppingcartFakerDialogComponent);
